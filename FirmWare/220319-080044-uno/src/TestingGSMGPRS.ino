...
//  String APN = "internet";
  if (checkConn()) {
    ...
    Serial.println(" ");
    Serial.println("There is not network or simCard not inserted.");
  }
...
//concatinate every bit after being compared to a character
String if_Available_read() {
  while (gsm.available()) {

    StringFromBuffer += checkAscii(gsm.read());
  }

  return StringFromBuffer;
}

//concatinate the message got from gsm
String if_Available_read(char ch) {
...
bool checkError(){
  ...
  while(gsm.available()){
    ...
    if(returnCharacter() == error[ecount]){
      fe+=1;
      ecount+=1;
      if(ecount == 4){
        ecount = 0;
...
bool internet_connection() {
  ...
  if (checkConn) {
    ...
    char error[] = "IP INITIAL";

    while(gsm.available()){
      
      if(returnCharacter() == error[count]){
        f+=1;
        count+=1;
        if(count == 4){
          count = 0;
    ...
    if(f >= 4){
      ...
      Serial.println("STATUS is NOT INITIAL");
      Serial.println("Running initializer");
      gsm.write(INIT);
      delay(3000);
      while(gsm.available()){
      ...
      Serial.println("Checking Status again..");
...
//    connect to the accesss point
    ...
    Serial.println("Get DN server name");
    gsm.write(GETDNSIP);
    delay(2000);
...
//    gsm.write(CMD);
    ...
    gsm.println(SENDTOSERVER+b);
    delay(2000);
    gsm.println("AT+CIPRDTIMER=2000");
    delay(3000);
